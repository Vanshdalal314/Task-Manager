...
const Layout = ({ user, onLogout }) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.get("https://task-manager-backend-tll4.onrender.com/api/tasks/gp", {
  ...
  const StatCard = ({ title, value, icon }) => (
    <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-blue-100 hover:shadow-md transition-all duration-300 hover:border-blue-200 group">
      <div className="flex items-center gap-2">
        <div className="p-1.5 rounded-lg bg-gradient-to-br from-blue-500/10 to-indigo-500/10 group-hover:from-blue-500/20 group-hover:to-blue-500/20">
          {icon}
        </div>
        <div className="min-w-0">
          <p className="text-lg sm:text-xl font-bold bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600 bg-clip-text text-transparent">
            {value}
          </p>
            ...
            <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-blue-100">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                <StatCard 
                  ...
                  icon={<Circle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-500" />} 
                />
                <StatCard 
                  title="Completed" 
                ...
                <StatCard
                  ...
                  icon={<Zap className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-500" />}
                />
              </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                {tasks.slice(0, 3).map((task) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2 sm:p-3 hover:bg-blue-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-blue-100"
                  >
