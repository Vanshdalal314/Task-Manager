        ...
        {STATS.map(({ key, label, icon: Icon, iconColor, borderColor = "border-blue-100", valueKey, textColor, gradient }) => (
          <div key={key} className={`${STAT_CARD} ${borderColor}`}>
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div className="min-w-0">
                <p className={`${VALUE_CLASS} ${gradient ? "bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600 bg-clip-text text-transparent" : textColor}`}>{stats[valueKey]}</p>
        ...
        <div className="space-y-4">
          {filteredTasks.length === 0 ? (
            <div className={EMPTY_STATE.wrapper}>
              <div className={EMPTY_STATE.iconWrapper}><CalendarIcon className="w-8 h-8 text-blue-500" /></div>
